...
extern double sg_seqmoderun_NowCumulant;//记录现在的累积量
...
static void BolusModeQuantity(gt_event_st * e) {
	...
	keyboardf.id=21;
	limit.toplimit = SequenMode.S1VTBI;
...
static void KEY_EVENT(gt_event_st * e) {
	...
	if(atoi(e->param) == 2) //up longdown
	{
		...
		press_Data.Pressure_newAD0 = sensorpack.DownPressure;
		SetNowcumulant(sg_seqmoderun_NowCumulant);//保存运行模式下的已入量值
		gt_disp_stack_load_scr_anim(GT_SCR_ID_93, GT_SCR_ANIM_TYPE_NONE, 500, 0, true);
		gt_event_send(SeqBolusRun,MY_EVENT_UPDATA,"1");
	}
}
...
gt_obj_st * gt_init_SeqBolus(void)
{
	...
	SeqBolus =  gt_obj_create(NULL);
	my_set_obj_title(SeqBolus,title[SystemSetting.langue][0]);
	gt_screen_set_bgcolor(SeqBolus, gt_color_hex(0x000000));

	gt_obj_add_event_cb(SeqBolus,KEY_EVENT,MY_KEY_EVENT,NULL);
	gt_obj_add_event_cb(SeqBolus,EventUpdata,MY_EVENT_UPDATA,NULL);
	
	/** img1 */
	img1 = gt_img_create(SeqBolus);
	...
	/** lab2 */
	lab2 = gt_label_create(SeqBolus);
	gt_obj_set_pos(lab2, 20, 60);
	gt_obj_set_size(lab2, 125, 35);
	...
	/** lab10 */
	lab10 = gt_label_create(SeqBolus);
	...
	/** lab11 */
	lab11 = gt_label_create(SeqBolus);
