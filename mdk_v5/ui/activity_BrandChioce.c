...
static void BrandShowpage(unsigned int Surplus)  //用来帮助显示最后一页
{
	switch(Surplus)
	{
		case 1:
...
static unsigned int sg_maxpage;
static unsigned int sg_nowpage;
static unsigned int sg_surplus;
...
static void Choice(gt_event_st * e) {
	...
	*/ 
	...
	strcpy(InfusionNow.tubename,getTheTubeNodeName(tempnode));
	sprintf(buffer,"%s",InfusionNow.tubename);
	not_first_start_write_history_info(EA_Set,Ev_Tube,buffer);
	
	gt_disp_stack_go_back(1);
}

static void BrandUp(gt_event_st* e){
	char buff[30];
	if(sg_nowpage>1)
	{
		sg_nowpage--;
...
static void BrandDown(gt_event_st* e){
	char buff[30];
...
gt_obj_st * gt_init_BrandChoice(void)
{
	...
	BrandChoice =  gt_obj_create(NULL);
	my_set_obj_title(BrandChoice,title[SystemSetting.langue][0]);
	gt_screen_set_bgcolor(BrandChoice, gt_color_hex(0x000000));
	 
	#if 0
	gt_obj_add_event_cb(BrandChoice,one_sec_time,MY_ONE_SECONDS_TIME,NULL);
	#endif



	/** lab2 */
	lab2 = gt_label_create(BrandChoice);
	...
	/** lab3 */
	lab3 = gt_label_create(BrandChoice);
	...
	/** lab4 */
	lab4 = gt_label_create(BrandChoice);
	...
	/** lab5 */
	lab5 = gt_label_create(BrandChoice);
	...
	/** lab6 */
	lab6 = gt_label_create(BrandChoice);
	...
	/** lab7 */
	lab7 = gt_label_create(BrandChoice);
	...
	/** rect2 */
	rect2 = gt_rect_create(BrandChoice);
	...
	/** rect3 */
	rect3 = gt_rect_create(BrandChoice);
	...
	/** rect4 */
	rect4 = gt_rect_create(BrandChoice);
	...
	/** rect5 */
	rect5 = gt_rect_create(BrandChoice);
	...
	/** rect6 */
	rect6 = gt_rect_create(BrandChoice);
	...
	/** rect7 */
	rect7 = gt_rect_create(BrandChoice);
	...
	/** svg1 */
	svg1 = gt_img_create(BrandChoice);
	gt_obj_set_pos(svg1, 405, 230);
	gt_obj_set_size(svg1, 50, 50);
	gt_img_set_src(svg1, "f:img_65x65_arrow_circle_down_2_16.png");
	gt_obj_add_event_cb(svg1, BrandDown, GT_EVENT_TYPE_INPUT_RELEASED, NULL);

	/** svg2 */
	svg2 = gt_img_create(BrandChoice);
	gt_obj_set_pos(svg2, 405, 70);
	gt_obj_set_size(svg2, 50, 50);
	gt_img_set_src(svg2, "f:img_65x65_arrow_circle_up_2_15.png");
	gt_obj_add_event_cb(svg1, BrandUp, GT_EVENT_TYPE_INPUT_RELEASED, NULL);


	/** lab8 */
	lab8 = gt_label_create(BrandChoice);
