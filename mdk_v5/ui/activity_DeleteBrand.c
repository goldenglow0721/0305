...
void Brandaffirm(gt_event_st * e) {
	...
	*/ 
	Brandtemp= atoi(e->user_data);
	InfusionNode_t* tempnode = fineInfusionNodeByPosit(Brandtemp*sg_nowpage+2);
    //删除内容
	if(SystemSetting.langue==Chinese)
        removeInfusionNodeByName(tempnode->message.ch_name,false);
    else if(SystemSetting.langue==English)
        removeInfusionNodeByName(tempnode->message.en_name,false);
    else if(SystemSetting.langue==Spanish)
        removeInfusionNodeByName(tempnode->message.es_name,false);
    else if(SystemSetting.langue==Chinese2)
        removeInfusionNodeByName(tempnode->message.ch2_name,false);
    else if(SystemSetting.langue==Portuguese)
        removeInfusionNodeByName(tempnode->message.pu_name,false);
	...
	fineInfusionNode_six_ById(sg_nowpage*6-4);
	//更新内容
...
//static void press(gt_event_st * e){
//	dialog2_create((char *)title[SystemSetting.langue][1],e->user_data);
//}

gt_obj_st * _dialog2_init(gt_event_st * e) {
	...
	/** btn3 */
	btn3 = gt_btn_create(dialog2);
	gt_obj_set_pos(btn3, 135, 165);
	...
	/** btn4 */
	btn4 = gt_btn_create(dialog2);
	gt_obj_set_pos(btn4, 255, 165);
	...
	gt_obj_add_event_cb(btn4, Brandaffirm, GT_EVENT_TYPE_INPUT_PRESSED, e->user_data);


	/** lab12 */
	lab12 = gt_label_create(dialog2);
	gt_obj_set_pos(lab12, 120, 115);
	gt_obj_set_size(lab12, 240, 50);
...
gt_obj_st * gt_init_DeleteBrand(void)
{
	...
	InfusionNode_t* tempnode = fineInfusionNodeByPosit(3);
	InfusionNodelangue(tubename1,tempnode);

	tempnode=fineInfusionNodeByPosit(4);
	InfusionNodelangue(tubename2,tempnode);

	tempnode=fineInfusionNodeByPosit(5);
	InfusionNodelangue(tubename3,tempnode);

	tempnode=fineInfusionNodeByPosit(6);
	InfusionNodelangue(tubename4,tempnode);

	tempnode=fineInfusionNodeByPosit(7);
	InfusionNodelangue(tubename5,tempnode);

	tempnode=fineInfusionNodeByPosit(8);
	InfusionNodelangue(tubename6,tempnode);


	DeleteBrand =  gt_obj_create(NULL);
	my_set_obj_title(DeleteBrand,title[SystemSetting.langue][0]);
	gt_screen_set_bgcolor(DeleteBrand, gt_color_hex(0x000000));
	 
	#if 0
	gt_obj_add_event_cb(DeleteBrand,one_sec_time,MY_ONE_SECONDS_TIME,NULL);
	#endif

	/** img1 */
	img1 = gt_img_create(DeleteBrand);
	...
	/** lab2 */
	lab2 = gt_label_create(DeleteBrand);
	...
	/** lab3 */
	lab3 = gt_label_create(DeleteBrand);
	...
	/** lab4 */
	lab4 = gt_label_create(DeleteBrand);
	...
	/** lab5 */
	lab5 = gt_label_create(DeleteBrand);
	...
	/** lab6 */
	lab6 = gt_label_create(DeleteBrand);
	...
	/** lab7 */
	lab7 = gt_label_create(DeleteBrand);
	...
	/** rect2 */
	rect2 = gt_rect_create(DeleteBrand);
	...
	gt_obj_add_event_cb(rect2, _dialog2_init, GT_EVENT_TYPE_INPUT_RELEASED, "1");



	/** rect3 */
	rect3 = gt_rect_create(DeleteBrand);
	...
	gt_obj_add_event_cb(rect3, _dialog2_init, GT_EVENT_TYPE_INPUT_RELEASED, "2");



	/** rect4 */
	rect4 = gt_rect_create(DeleteBrand);
	...
	gt_obj_add_event_cb(rect4, _dialog2_init, GT_EVENT_TYPE_INPUT_RELEASED, "3");



	/** rect5 */
	rect5 = gt_rect_create(DeleteBrand);
	...
	gt_obj_add_event_cb(rect5, _dialog2_init, GT_EVENT_TYPE_INPUT_RELEASED, "4");



	/** rect6 */
	rect6 = gt_rect_create(DeleteBrand);
	...
	gt_obj_add_event_cb(rect6, _dialog2_init, GT_EVENT_TYPE_INPUT_RELEASED, "5");



	/** rect7 */
	rect7 = gt_rect_create(DeleteBrand);
	...
	gt_obj_add_event_cb(rect7, _dialog2_init, GT_EVENT_TYPE_INPUT_RELEASED, "6");



	/** svg1 */
	svg1 = gt_img_create(DeleteBrand);
	gt_obj_set_pos(svg1, 411, 230);
	...
	/** svg2 */
	svg2 = gt_img_create(DeleteBrand);
	gt_obj_set_pos(svg2, 411, 70);
	...
	/** lab8 */
	lab8 = gt_label_create(DeleteBrand);
