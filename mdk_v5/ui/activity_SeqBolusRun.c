...
SeqBolusModeRun SeqBolus_ModeRun;
...
static void btn1_0_cb(gt_event_st * e) {
	...
	gt_event_send(SeqBolusRun,MY_MOTOR_RUN,NULL);
...
static void btn2_0_cb(gt_event_st * e) {
	...
	ClearAlarm(ALARM_SYSTEM_FAILURE);
	ClearAlarm(ALARM_INFUSION_OFF);
	ClearAlarm(ALARM_NEAR_DONE); SetNerDoneFlag(0);
	...
	if(MotorControl.status != MOTOR_BACK)
	{
		...
		if(*Bolus_ModeRun.quantity != 0)
		{
			*Bolus_ModeRun.quantity = sg_quanbegin - sg_NowCumulant;
			...
			sprintf(buffer,"%.2f",*Bolus_ModeRun.quantity);
			gt_label_set_text(lab11,buffer);

			sprintf(buffer,"%.2f",sg_NowCumulant);
			...
			if((*Bolus_ModeRun.quantity)<=0.0)
			{
				*Bolus_ModeRun.quantity = 0.0;
				...
				if(SystemMaintain.KVOState)
				{
					...
					gt_event_send(btn2,GT_EVENT_TYPE_INPUT_RELEASED,"1");
				}
				else
				{
					if(MotorControl.status == MOTOR_RUN)
					{
						...
						sprintf(buffer,"%.2f",GetNowcumulant()+sg_NowCumulant); 
						gt_label_set_text(lab20,buffer);
...
gt_obj_st * gt_init_SeqBolusRun(void)
{
	...
	SeqBolusRun =  gt_obj_create(NULL);
	my_set_obj_title(SeqBolusRun,title[SystemSetting.langue][0]);
	gt_screen_set_bgcolor(SeqBolusRun, gt_color_hex(0x000000));

	gt_obj_add_event_cb(SeqBolusRun, MotorRun,MY_MOTOR_RUN, NULL);
	
	gt_obj_add_event_cb(SeqBolusRun, Timer_OneSecond,MY_ONE_SECONDS_TIME, NULL);

	gt_obj_add_event_cb(SeqBolusRun,updata_EVENT,MY_EVENT_UPDATA,NULL);

	/** 停止快进 */
	btn1 = gt_btn_create(SeqBolusRun);
	...
	/** 快进流速 */
	lab2 = gt_label_create(SeqBolusRun);
	...
	/** lab3 */
	lab3 = gt_label_create(SeqBolusRun);
	...
	/** lab4 */
	lab4 = gt_label_create(SeqBolusRun);
	...
	/** rect3 */
	rect3 = gt_rect_create(SeqBolusRun);
	...
	/** btn2 */
	btn2 = gt_btn_create(SeqBolusRun);
	...
	/** 快进已入量 */
	lab17 = gt_label_create(SeqBolusRun);
	...
	/** lab18 */
	lab18 = gt_label_create(SeqBolusRun);
	...
	/** 快进已入量 */
	lab19 = gt_label_create(SeqBolusRun);
	...
	sprintf(buffer,"%.2f",sg_thiscumulant);
	gt_label_set_text(lab19, buffer);


	/** 品牌 */
	lab5 = gt_label_create(SeqBolusRun);
	...
	/** 累积量 */
	lab6 = gt_label_create(SeqBolusRun);
	...
	/** 压力 */
	lab7 = gt_label_create(SeqBolusRun);
	...
	/** lab8 */
	lab8 = gt_label_create(SeqBolusRun);
	...
	/** 压力单位 */
	lab9 = gt_label_create(SeqBolusRun);
	...
	/** 输液器品牌名 */
	lab10 = gt_label_create(SeqBolusRun);
	...
	/** 待入量 */
	lab11 = gt_label_create(SeqBolusRun);
	...
	/** img1 */
	img1 = gt_img_create(SeqBolusRun);
	...
	/** 最大压力值 */
	lab12 = gt_label_create(SeqBolusRun);
	...
	/** 当前的压力值 */
	lab13 = gt_label_create(SeqBolusRun);
	...
	/** 压力条 */
	bar1 = gt_progress_bar_create(SeqBolusRun);
	...
	/** 剩余时间 */
	lab14 = gt_label_create(SeqBolusRun);
	...
	/** lab15 */
	lab15 = gt_label_create(SeqBolusRun);
	...
	/** lab16 */
	lab16 = gt_label_create(SeqBolusRun);
	...
	/** rect4 */
	rect4 = gt_rect_create(SeqBolusRun);
	...
	/** rect5 */
	rect5 = gt_rect_create(SeqBolusRun);
	...
	/** lab21 */
	lab21 = gt_label_create(SeqBolusRun);
	...
	/** 累积量 */
	lab20 = gt_label_create(SeqBolusRun);
