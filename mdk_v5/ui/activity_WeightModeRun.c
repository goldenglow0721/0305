...
WeightMode_Run WeightMode_run;

static const char title[5][11][50]={
	"体重模式",
	"快进",
	"剂量速度",
	"品牌",
...
double sg_weightmoderun_NowCumulant;
...
static void bolus(gt_event_st * e) {
	...
	if(*WeightMode_run.speed >= SystemMaintain.MaxSpeed){return ;}
...
static void ChangeDoseSpeed(gt_event_st * e) {
	double maxDoseSpeed;
	maxDoseSpeed = (SystemMaintain.MaxSpeed * WeightMode.Medicine)/(WeightMode.Body * WeightMode.Liquid);
	if(GetLockFlag()){return ;}else{SetAutoLockTime(ParamSetting.LockTime);}
	if(GetNowAlarmLeve12()==H_ALARM){return ;}
	strcpy(keyboardf.title,title[SystemSetting.langue][2]);
	keyboardf.data = WeightMode_run.DoseSpeed;
	keyboardf.id=18;
	limit.lowerlimit=0.10;
	limit.toplimit=maxDoseSpeed;
...
static void MotorRun(gt_event_st * e){
	...
	sg_quanbegin = *WeightMode_run.quantity;
	sg_cumlant   = GetNowcumulant();

	//计算超声检查时间
	UltrasonicTimeMS=(unsigned int)(REQUIRED_TIMES(*WeightMode_run.speed,ParamSetting.BubbleSize)/2);  
	if(UltrasonicTimeMS<=1)
		UltrasonicTimeMS=1;
	//计算累计气泡量在当前速度下速度的时间毫秒
	AcBubTimeNumMs=(unsigned int)CAL_SPEED_ACC_BUB_TIME_MS_NUM(*WeightMode_run.speed,ParamSetting.AccumBubbleSize);
	
	SetNerDoneFlag(0);
	//计算速度
	tempdata =CalSpeed(*WeightMode_run.speed); 
	TMC2226_MotorRun(1,0,tempdata);
	//设置运行条运行速度
	DEL_TIMER_VAL(run_progress);
	run_progress = _gt_timer_create(run_progress_func,SetProgressRunSpeed(*WeightMode_run.speed),NULL);
...
static void Timer_OneSecond(gt_event_st * e){
	char buffer[50];
	//计算剩余时间
	if(*WeightMode_run.quantity != 0 )
	{
		sg_surplustime = CalSurplusTime(*WeightMode_run.quantity,*WeightMode_run.speed);
...
static void Timer_300mSecond(struct _gt_timer_s *timer){
	...
	{
		sg_weightmoderun_NowCumulant = CalCumulant(MotorControl.anglesum,*WeightMode_run.speed);
		//printf("num = %f\n",MotorControl.anglesum);
		if(sg_weightmoderun_NowCumulant <= 0.0)
			sg_weightmoderun_NowCumulant = 0.0;
		if(*WeightMode_run.quantity != 0)
		{
			*WeightMode_run.quantity = sg_quanbegin - sg_weightmoderun_NowCumulant;
			sprintf(buffer,"%.2f",(float)*WeightMode_run.quantity);
			gt_label_set_text(lab25, buffer);

			sprintf(buffer,"%.2f",(float)(sg_cumlant+sg_weightmoderun_NowCumulant)); 
			...
			//输液完成
			if((*WeightMode_run.quantity)<=0.0)
			{
				*WeightMode_run.quantity = 0.0;
				sg_weightmoderun_NowCumulant = sg_quanbegin;
				...
				else
				{
					if(MotorControl.status == MOTOR_RUN)
					{
						...
						sprintf(buffer,"%.2f",(float)(GetNowcumulant()+sg_weightmoderun_NowCumulant)); 
						gt_label_set_text(lab11,buffer);
...
static void EventUpdata(gt_event_st * e)
{
	...
	if(atoi(e->param) == 18)  //修改完剂量速度出来的
	{
		
		UpdateWeightModeTheDisplay(CHANGE_DOS);
		//更新速度显示
		sprintf(buffer,"%.2f",*WeightMode_run.DoseSpeed);
		gt_label_set_text(lab4, buffer);
		sprintf(buffer,"%.2f",*WeightMode_run.speed);
		gt_label_set_text(lab19, buffer);


		//更新时间显示
		if(SpeedMode.time.hour>99)
		{
			WeightMode_run.time->hour=100;
			WeightMode_run.time->min=0;
			WeightMode_run.time->sec=0;			
			sprintf(buffer,">%d:%d:%d", 99, 59, 59);
		}
		sprintf(buffer,"%d:%d:%d",WeightMode_run.time->hour, WeightMode_run.time->min,WeightMode_run.time->sec);
		gt_label_set_text(lab16, buffer);
		//写历史记录
		sprintf(buffer,"%.2fml/h",(float)*WeightMode_run.speed);
		not_first_start_write_history_info(EA_Amend,Ev_SetSpeed,buffer);
		sprintf(buffer,"%.2fml/h",(float)*WeightMode_run.DoseSpeed);
		not_first_start_write_history_info(EA_Amend,Ev_SetDoseSpeed,buffer);
...
gt_obj_st * gt_init_WeightModeRun(void)
{
	...
	//写历史记录
	sprintf(buffer,"%.2fml/h,%.2fml.  %s",(float)*WeightMode_run.speed,(float)*WeightMode_run.quantity,InfusionNow.drugname);
	not_first_start_write_history_info(EA_start,Ev_StarInjection,buffer);

	pressureAD = 0;

	WeightModeRun =  gt_obj_create(NULL);
	my_set_obj_title(WeightModeRun,title[SystemSetting.langue][0]);
	gt_screen_set_bgcolor(WeightModeRun, gt_color_hex(0x000000));
	gt_obj_add_event_cb(WeightModeRun,EventUpdata,MY_EVENT_UPDATA,NULL);
	
	gt_obj_add_event_cb(WeightModeRun, EventUpdata,MY_EVENT_UPDATA, NULL);
	gt_obj_add_event_cb(WeightModeRun, MotorRun,MY_MOTOR_RUN, NULL);
	gt_obj_add_event_cb(WeightModeRun, Timer_OneSecond,MY_ONE_SECONDS_TIME, NULL);

	/** btn1 */
	btn1 = gt_btn_create(WeightModeRun);
	gt_obj_set_pos(btn1, 0, 180);
	...
	/** lab2 */
	lab2 = gt_label_create(WeightModeRun);
	gt_obj_set_pos(lab2, 0, 50);
	gt_obj_set_size(lab2, 120, 35);
	...
	/** lab3 */
	lab3 = gt_label_create(WeightModeRun);
	gt_obj_set_pos(lab3, 134, 50);
	gt_obj_set_size(lab3, 104, 25);
	gt_label_set_font_color(lab3, gt_color_hex(0xffffff));
	gt_label_set_font_size(lab3, 20);
	gt_label_set_font_family_en(lab3, 30);
	gt_label_set_font_family_cn(lab3, 10);
	gt_label_set_font_family_fl(lab3, 50);
	gt_label_set_font_align(lab3, GT_ALIGN_RIGHT_MID);
	gt_label_set_text(lab3, GetDoseUintString(WeightMode.DS_Uint));


	/** lab4 */
	lab4 = gt_label_create(WeightModeRun);
	gt_obj_set_pos(lab4, 0, 95);
	...
	sprintf(buffer,"%.2f",(float)*WeightMode_run.DoseSpeed);
	gt_label_set_text(lab4,buffer);


	/** lab5 */
	lab5 = gt_label_create(WeightModeRun);
	...
	/** lab6 */
	lab6 = gt_label_create(WeightModeRun);
	gt_obj_set_pos(lab6, 252, 135);
	gt_obj_set_size(lab6, 85, 26);
	...
	/** lab7 */
	lab7 = gt_label_create(WeightModeRun);
	gt_obj_set_pos(lab7, 252, 230);
	gt_obj_set_size(lab7, 72, 25);
	...
	/** lab9 */
	lab9 = gt_label_create(WeightModeRun);
	...
	gt_label_set_font_family_en(lab9, 10);
	...
	/** lab10 */
	lab10 = gt_label_create(WeightModeRun);
	gt_obj_set_pos(lab10, 330, 20);
	...
	/** lab11 */
	lab11 = gt_label_create(WeightModeRun);
	gt_obj_set_pos(lab11, 410, 155);
	gt_obj_set_size(lab11, 70, 25);
	gt_label_set_font_color(lab11, gt_color_hex(0xffffff));
	gt_label_set_font_size(lab11, 20);
	gt_label_set_font_family_en(lab11, 30);
	gt_label_set_font_family_cn(lab11, 10);
	...
	/** img1 */
	img1 = gt_img_create(WeightModeRun);
	gt_obj_set_pos(img1, 252, 252);
	gt_obj_set_size(img1, 20, 30);
	gt_img_set_src(img1, "f:img_20x30_46_keyboard_shift.png");


	/** rect3 */
	rect3 = gt_rect_create(WeightModeRun);
	gt_obj_set_pos(rect3, 0, 85);
	gt_obj_set_size(rect3, 240, 65);
	...
	gt_obj_add_event_cb(rect3, ChangeDoseSpeed, GT_EVENT_TYPE_INPUT_RELEASED, NULL);
	


	/** lab12 */
	lab12 = gt_label_create(WeightModeRun);
	gt_obj_set_pos(lab12, 275, 259);
	gt_obj_set_size(lab12, 78, 24);
	gt_label_set_font_color(lab12, gt_color_hex(0xffffff));
	gt_label_set_font_size(lab12, 20);
	gt_label_set_font_family_en(lab12, 30);
	gt_label_set_font_family_cn(lab12, 10);
	...
	/** lab13 */
	lab13 = gt_label_create(WeightModeRun);
	gt_obj_set_pos(lab13, 417, 259);
	gt_obj_set_size(lab13, 60, 25);
	gt_label_set_font_color(lab13, gt_color_hex(0xffffff));
	gt_label_set_font_size(lab13, 20);
	gt_label_set_font_family_en(lab13, 30);
	gt_label_set_font_family_cn(lab13, 10);
	...
	/** bar1 */
	bar1 = gt_progress_bar_create(WeightModeRun);
	gt_obj_set_pos(bar1, 252, 290);
	gt_obj_set_size(bar1, 215, 20);
	...
	/** lab14 */
	lab14 = gt_label_create(WeightModeRun);
	...
	/** lab16 */
	lab16 = gt_label_create(WeightModeRun);
	...
	/** btn2 */
	btn2 = gt_btn_create(WeightModeRun);
	gt_obj_set_pos(btn2, 135, 180);
	...
	gt_obj_add_event_cb(btn2, Pause, GT_EVENT_TYPE_INPUT_RELEASED, NULL);


	#if (VET_USE || !CH_USE) //兽用或者国外使用
	/** 药物名 */
	lab17 = gt_label_create(WeightModeRun);
	...
	/** rect7 */
	rect7 = gt_rect_create(WeightModeRun);
	...
	/** svg1 */
	svg1 = gt_img_create(WeightModeRun);
	gt_obj_set_pos(svg1, 7, 220);
	...
	/** svg2 */
	svg2 = gt_img_create(WeightModeRun);
	gt_obj_set_pos(svg2, 7, 220);
	...
	/** rect4 */
	rect4 = gt_rect_create(WeightModeRun);
	...
	/** rect5 */
	rect5 = gt_rect_create(WeightModeRun);
	...
	/** lab18 */
	lab18 = gt_label_create(WeightModeRun);
	gt_obj_set_pos(lab18, 0, 155);
	gt_obj_set_size(lab18, 45, 24);
	...
	/** lab19 */
	lab19 = gt_label_create(WeightModeRun);
	gt_obj_set_pos(lab19, 39, 155);
	gt_obj_set_size(lab19, 60, 24);
	gt_label_set_font_color(lab19, gt_color_hex(0xffffff));
	gt_label_set_font_size(lab19, 20);
	gt_label_set_font_family_cn(lab19, 10);
	gt_label_set_font_family_en(lab19, 30);
	gt_label_set_font_family_fl(lab19, 50);
	gt_label_set_font_align(lab19, GT_ALIGN_LEFT);
	sprintf(buffer,"%.1f",(float)*WeightMode_run.speed);
	gt_label_set_text(lab19,buffer);


	/** lab20 */
	lab20 = gt_label_create(WeightModeRun);
	gt_obj_set_pos(lab20, 89, 155);
	gt_obj_set_size(lab20, 40, 24);
	...
	gt_label_set_text(lab20, "mL/h");


	/** lab21 */
	lab21 = gt_label_create(WeightModeRun);
	gt_obj_set_pos(lab21, 133, 155);
	gt_obj_set_size(lab21, 44, 24);
	gt_label_set_font_color(lab21, gt_color_hex(0xffffff));
	gt_label_set_font_size(lab21, 20);
	gt_label_set_font_family_cn(lab21, 10);
	gt_label_set_font_family_en(lab21, 30);
	gt_label_set_font_family_fl(lab21, 50);
	gt_label_set_font_align(lab21, GT_ALIGN_LEFT);
	gt_label_set_text(lab21, title[SystemSetting.langue][9]);


	/** lab22 */
	lab22 = gt_label_create(WeightModeRun);
	gt_obj_set_pos(lab22, 177, 155);
	gt_obj_set_size(lab22, 50, 24);
	gt_label_set_font_color(lab22, gt_color_hex(0xffffff));
	gt_label_set_font_size(lab22, 20);
	gt_label_set_font_family_cn(lab22, 10);
	gt_label_set_font_family_en(lab22, 30);
	gt_label_set_font_family_fl(lab22, 50);
	gt_label_set_font_align(lab22, GT_ALIGN_LEFT);
	sprintf(buffer,"%.1f",(float)*WeightMode_run.Body);
	gt_label_set_text(lab22, buffer);


	/** lab23 */
	lab23 = gt_label_create(WeightModeRun);
	gt_obj_set_pos(lab23, 219, 155);
	gt_obj_set_size(lab23, 21, 24);
	gt_label_set_font_color(lab23, gt_color_hex(0xffffff));
	gt_label_set_font_size(lab23, 20);
	gt_label_set_font_family_cn(lab23, 10);
	gt_label_set_font_family_en(lab23, 30);
	gt_label_set_font_family_fl(lab23, 50);
	gt_label_set_font_align(lab23, GT_ALIGN_LEFT);
	gt_label_set_text(lab23, "kg");


	/** lab24 */
	lab24 = gt_label_create(WeightModeRun);
	gt_obj_set_pos(lab24, 252, 86);
	gt_obj_set_size(lab24, 85, 26);
	gt_label_set_font_color(lab24, gt_color_hex(0xffffff));
	gt_label_set_font_size(lab24, 20);
	gt_label_set_font_family_cn(lab24, 10);
	gt_label_set_font_family_en(lab24, 30);
	gt_label_set_font_family_fl(lab24, 50);
	gt_label_set_font_align(lab24, GT_ALIGN_LEFT);
	gt_label_set_text(lab24, title[SystemSetting.langue][10]);


	/** lab25 */
	lab25 = gt_label_create(WeightModeRun);
	gt_obj_set_pos(lab25, 410, 110);
	gt_obj_set_size(lab25, 70, 25);
	gt_label_set_font_color(lab25, gt_color_hex(0xffffff));
	gt_label_set_font_size(lab25, 20);
	gt_label_set_font_family_en(lab25, 30);
	gt_label_set_font_family_cn(lab25, 10);
	gt_label_set_font_family_fl(lab25, 50);
	gt_label_set_font_align(lab25, GT_ALIGN_RIGHT);
	sprintf(buffer,"%.2f",(float)*WeightMode_run.quantity);
	gt_label_set_text(lab25, buffer);


	/** lab26 */
	lab26 = gt_label_create(WeightModeRun);
	gt_obj_set_pos(lab26, 418, 86);
	gt_obj_set_size(lab26, 60, 25);
	gt_label_set_font_color(lab26, gt_color_hex(0xffffff));
	gt_label_set_font_size(lab26, 20);
	gt_label_set_font_family_en(lab26, 30);
	gt_label_set_font_family_cn(lab26, 10);
	gt_label_set_font_family_fl(lab26, 50);
	gt_label_set_font_align(lab26, GT_ALIGN_RIGHT);
	gt_label_set_text(lab26, "mL");

	/** img3 */
	img3 = gt_img_create(WeightModeRun);
	gt_obj_set_pos(img3, 10, 280);
	gt_obj_set_size(img3, 214, 30);
	gt_img_set_src(img3, "f:img_214x30_Runstrip.png");

	/** img2 */
	img2 = gt_img_create(WeightModeRun);
