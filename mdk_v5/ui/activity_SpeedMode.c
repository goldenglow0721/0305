...
static void SpeedChange(gt_event_st * e) {
	...
	keyboardf.data = &SpeedMode.speed;
	limit.toplimit=SystemMaintain.MaxSpeed;
	limit.lowerlimit = 0.01;
	keyboardf.id=3;
	gt_disp_stack_load_scr_anim(GT_SCR_ID_81, GT_SCR_ANIM_TYPE_NONE, 50, 0, false);
}
...
static void ChangeQuantity(gt_event_st * e) {
	...
	keyboardf.data = &SpeedMode.quantity;
...
static void btn1_0_cb(gt_event_st * e) {
	...
	#if 1  //运行前检测
	if(SpeedMode.speed==0){
		dialog1_create("请输入流速");
		return;
	}
	...
	#endif
	speed_time_run.title = (char*)&title[SystemSetting.langue][0];
	speed_time_run.quantity = &SpeedMode.quantity;
	speed_time_run.speed 	= &SpeedMode.speed;
	speed_time_run.time		= &SpeedMode.time;
	speed_time_run.init_func_cb = gt_init_SpeedMode;
...
static void EventUpdata(gt_event_st * e)
{
	...
	if(atoi(e->param) == 3)  //修改完速度出来的
	{
		
		UpdateSpeedModeTheDisplay(CHANGE_Speed,&SpeedMode.speed,&SpeedMode.quantity,&SpeedMode.time);
		//更新速度显示
		sprintf(buffer,"%.2f",(float)SpeedMode.speed);
		gt_label_set_text(lab5, buffer);
		if(SpeedMode.time.hour>99)
		{
			SpeedMode.time.hour=100;
			SpeedMode.time.min=0;
			SpeedMode.time.sec=0;			
			sprintf(buffer,">%d:%d:%d", 99, 59, 59);
		}
		else
		{
			if(SpeedMode.quantity!=0.0 && SpeedMode.speed!=0.0)
			{
				sprintf(buffer,"%02d:%02d:%02d", SpeedMode.time.hour, SpeedMode.time.min, SpeedMode.time.sec);
			}
			else
			{
	...
	if(atoi(e->param) == 4)  //修改完待入量出来的
	{
		UpdateSpeedModeTheDisplay(CHANGE_QUAN,&SpeedMode.speed,&SpeedMode.quantity,&SpeedMode.time);
		//更新待入量
		sprintf(buffer,"%.2f",(float)SpeedMode.quantity);
...
{
	char buffer[50];
	UiSpeedMode =  gt_obj_create(NULL);
	my_set_obj_title(UiSpeedMode,title[SystemSetting.langue][0]);
	gt_obj_add_event_cb(UiSpeedMode,EventUpdata,MY_EVENT_UPDATA,NULL);
	gt_obj_add_event_cb(UiSpeedMode,one_sec_timer,MY_ONE_SECONDS_TIME,NULL);
	 

	gt_screen_set_bgcolor(UiSpeedMode, gt_color_hex(0x000000));



	/** img1 */
	img1 = gt_img_create(UiSpeedMode);
	...
	/** lab2 */
	lab2 = gt_label_create(UiSpeedMode);
	...
	/** lab3 */
	lab3 = gt_label_create(UiSpeedMode);
	...
	/** lab4 */
	lab4 = gt_label_create(UiSpeedMode);
	...
	/** rect2 */
	rect2 = gt_rect_create(UiSpeedMode);
	...
	gt_obj_add_event_cb(rect2, SpeedChange, GT_EVENT_TYPE_INPUT_RELEASED, NULL);


	/** rect3 */
	rect3 = gt_rect_create(UiSpeedMode);
	...
	/** rect4 */
	rect4 = gt_rect_create(UiSpeedMode);
	...
	/** lab5 */
	lab5 = gt_label_create(UiSpeedMode);
	...
	if(SpeedMode.speed)
		sprintf(buffer,"%.2f",SpeedMode.speed);
	else
		sprintf(buffer,"--");
	gt_label_set_text(lab5, buffer);
	...
	/** lab6 */
	lab6 = gt_label_create(UiSpeedMode);
	...
	/** rect5 */
	rect5 = gt_rect_create(UiSpeedMode);
	...
	gt_obj_add_event_cb(rect5, SpeedChange, GT_EVENT_TYPE_INPUT_RELEASED, NULL);


	/** lab7 */
	lab7 = gt_label_create(UiSpeedMode);
	...
	if(SpeedMode.quantity)
		sprintf(buffer,"%.2lf",SpeedMode.quantity);
	else
		sprintf(buffer,"--");
	gt_label_set_text(lab7, buffer);
	...
	/** lab8 */
	lab8 = gt_label_create(UiSpeedMode);
	...
	/** rect6 */
	rect6 = gt_rect_create(UiSpeedMode);
	...
	/** lab10 */
	lab10 = gt_label_create(UiSpeedMode);
	...
	/** lab11 */
	lab11 = gt_label_create(UiSpeedMode);
	...
	/** rect7 */
	rect7 = gt_rect_create(UiSpeedMode);
	...
	/** btn1 */
	btn1 = gt_btn_create(UiSpeedMode);
	...
	/** btn2 */
	btn2 = gt_btn_create(UiSpeedMode);
