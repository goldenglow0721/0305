...
//获取当前的屏幕id
gt_scr_id_t my_get_now_scr_id(void)
{
    _gt_gc_scr_st * scr_info = _gt_gc_get_scr_info();
...
//判断页面是否存在
bool my_judge_obj_exists(gt_scr_id_t id)
{
    gt_stack_item_st result;
    gt_scr_stack_item_st need_find={
        .current_scr_id = id,
    };
   result =  gt_scr_stack_has_before(&need_find);
   if(result.data == NULL)
   {
       return  false;
   }
